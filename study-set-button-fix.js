(()=>{
  ...
  const persist=()=>{
    const d=getData();
    // Keep the live planner object and the persisted planner object identical.
    // Do NOT call savePlannerData here: that legacy saver serializes an older
    // planner schema and can overwrite the newly-added studySets immediately.
    ...
    try{
      ...
      return Array.isArray(stored.studySets)&&stored.studySets.some(s=>s&&String(s.id)===String(d.studySets?.at(-1)?.id));
    }catch(e){
      console.error('Study Set persistence failed:',e);
      return false;
    }
  };
  const forceLibraryCard=(set)=>{
    ...
    article.innerHTML=`<div class="study-set-icon">${icon}</div><div class="study-set-main"><div class="study-set-top"><div><h3>${esc(set.name)}</h3><div class="study-set-meta">${esc(set.subject||'No subject')} · ${typeLabel}</div></div></div>${set.description?`<p>${esc(set.description)}</p>`:''}${set.unit?`<span class="study-set-unit">${esc(set.unit)}</span>`:''}<div class="study-set-footer"><span>${Array.isArray(set.items)?set.items.length:0} items</span><div><button class="small-button" type="button" data-study-fix-open="${esc(set.id)}">Open</button></div></div></div>`;
    article.querySelector('[data-study-fix-open]').onclick=e=>{e.preventDefault();e.stopPropagation();if(typeof window.openStudySet==='function')window.openStudySet(set.id);else console.warn('openStudySet is not available yet');};
