...
(function installDashboardStudyLoad(){
    ...
    function render(){
        ...
        rows.forEach(row=>{
            const item=document.createElement('div');item.className='study-load-row';item.style.setProperty('--subject-color',row.subject.colour||'var(--planner-accent,#304b8a)');item.tabIndex=0;item.setAttribute('role','button');item.setAttribute('aria-label',`Open ${row.subject.name||'subject'}`);
            ...
            item.onclick=()=>openSubject(row.subject.name);item.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();openSubject(row.subject.name);}};
