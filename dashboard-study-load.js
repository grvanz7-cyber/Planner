...
(function installDashboardStudyLoad(){
    ...
    function render(){
        ...
        rows.forEach(row=>{
            const item=document.createElement('div');item.className='study-load-row';item.style.setProperty('--subject-color',row.subject.colour||'var(--planner-accent,#304b8a)');
            ...
            item.querySelector('.study-load-emoji').textContent=row.subject.emoji||'📚';item.querySelector('.study-load-subject strong').textContent=row.subject.name||'Untitled subject';item.querySelector('.study-load-count').textContent=`${row.schoolCount} school · ${row.count} total`;
            item.querySelector('.study-load-bar span').style.width=`${Math.max(0,(row.schoolCount/max)*100)}%`;
            item.querySelector('.study-load-next').textContent=row.next?`Next: ${row.next.name||'Untitled'} · ${String(row.next.dueDate).slice(0,10)}`:'Nothing scheduled';
