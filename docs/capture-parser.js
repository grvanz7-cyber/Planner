(() => {
  ...
  function parse(text){
    ...
    if(!subject){
      const aliasList=Object.entries(aliases).flatMap(([name,words])=>words.map(word=>({name,word}))).sort((a,b)=>b.word.length-a.word.length);
      for(const item of aliasList){if(tokenRegex(item.word).test(remaining)){subject=subjects.find(s=>s.name.toLowerCase()===item.name.toLowerCase())||null;if(subject)remaining=remaining.replace(tokenRegex(item.word)," ");break;}}
    }
    let unitNumber=null;
    ...
    remaining=remaining.replace(/\b(1h30|2h|1h|15m|30m|45m)\b/i,value=>{estimatedWorkload=value.toLowerCase();return " ";});
    remaining=remaining.replace(/\b(15|30|45)\s*minutes?\b/i,(_,n)=>{estimatedWorkload=`${n}m`;return " ";});
    remaining=remaining.replace(/\b1\s*hour\s*30\s*minutes?\b/i,()=>{estimatedWorkload="1h30";return " ";});
    remaining=remaining.replace(/\b(1|2)\s*hours?\b/i,(_,n)=>{estimatedWorkload=`${n}h`;return " ";});
    let type="Assignment";
    ...
    let priority=null;
    if(/\bhigh priority\b/i.test(remaining)){priority="High";remaining=remaining.replace(/\bhigh priority\b/i," ");}
    else if(/\blow priority\b/i.test(remaining)){priority="Low";remaining=remaining.replace(/\blow priority\b/i," ");}
    let status=null;
    const statuses=["in progress","not started","finished","submitted","graded"];
    for(const value of statuses){const re=new RegExp(`\\b${escapeRegExp(value)}\\b`,"i");if(re.test(remaining)){status=value.replace(/^./,c=>c.toUpperCase());remaining=remaining.replace(re," ");break;}}
  ...
  function createFromCapture(text){
    ...
    if(!parsed.subjectId){addCaptureNotification(`Could not identify a subject for “${text}”.`);return null;}
    ...
    if(parsed.unitNumber!=null&&!unit)addCaptureNotification(`${assignment.name} needs Unit ${parsed.unitNumber} to be checked.`);
  ...
  function addCaptureNotification(message){
    ...
    data.notifications.push({id:`notif_${Date.now()}_${Math.random().toString(36).slice(2,7)}`,type:"Confirmation",message,read:false,resolved:false,createdAt:new Date().toISOString()});
