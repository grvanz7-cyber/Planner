(() => {
  ...
  function active() {
    if (SUBJECTS_NAV) SUBJECTS_NAV.classList.add("active");
  }
  ...
  function renderSubjects() {
    ...
    PAGE.innerHTML = `<div class="subjects-feature">
      ...
      ${subjects.length ? `<div class="subjects-feature-grid">${subjects.map(card).join("")}</div>` : `<div class="subjects-feature-empty"><strong>No subjects yet</strong>Add your first subject to get started.</div>`}
    </div>`;
  ...
  function card(s) {
    return `<article class="subjects-feature-card" data-subject-id="${esc(s.id)}">
      <div class="subjects-feature-icon" style="background:${esc(s.color || "#e5ede7")}">${esc(s.icon || "📚")}</div>
      <h3>${esc(s.name)}</h3>
      <div class="subjects-feature-meta">Current grade: —<br>Current unit: —</div>
    </article>`;
  }

  function showAdd() {
    const b = document.createElement("div");
    b.className = "subjects-feature-modal-backdrop";
    b.innerHTML = `<div class="subjects-feature-modal" role="dialog" aria-modal="true" aria-label="Add Subject">
      <h2>Add Subject</h2>
      <form class="subjects-feature-form" id="subjectsFeatureForm">
        ...
        <div class="subjects-feature-field"><label>Colour</label><div class="subjects-feature-colours" id="subjectColor"><button type="button" data-colour="#e5ede7" class="selected" style="background:#e5ede7"></button><button type="button" data-colour="#dce8f5" style="background:#dce8f5"></button><button type="button" data-colour="#f4e1e1" style="background:#f4e1e1"></button><button type="button" data-colour="#eee4d1" style="background:#eee4d1"></button><button type="button" data-colour="#e4ddf2" style="background:#e4ddf2"></button><button type="button" data-colour="#dcece3" style="background:#dcece3"></button></div></div>
        <div class="subjects-feature-field"><label>Academic period</label><div class="subjects-feature-periods"><button type="button" data-period="Semester 1" class="selected">Semester 1</button><button type="button" data-period="Semester 2">Semester 2</button></div></div>
    ...
    let selectedColour = "#e5ede7";
    let selectedPeriod = "Semester 1";
    ...
    b.querySelector("#subjectCancel").onclick = () => b.remove();
    b.onclick = e => { if (e.target === b) b.remove(); };
    b.querySelector("form").onsubmit = e => {
      e.preventDefault();
      const s = window.PlannerData.create("subjects", { name: b.querySelector("#subjectName").value.trim(), icon: b.querySelector("#subjectIcon").value.trim() || "📚", color: selectedColour, academicPeriod: selectedPeriod });
      b.remove();
      renderSubject(s.id);
    };
  }
  ...
  function renderSubject(id) {
    active();
    const s = window.PlannerData.find("subjects", id);
    if (!s) { location.hash = "subjects"; return; }
    const units = data().units.filter(u => u.subjectId === s.id).sort((a, b) => (Number(a.number) || 999) - (Number(b.number) || 999));
    const current = units.find(u => u.status === "Current");
    PAGE.innerHTML = `<div class="subjects-feature">
      <div class="subjects-feature-detail-header">
        <button class="subjects-feature-secondary" id="subjectBack">← Back</button>
        <div class="subjects-feature-subject-heading"><div class="subjects-feature-icon" style="background:${esc(s.color || "#e5ede7")}">${esc(s.icon || "📚")}</div><div><h2>${esc(s.name)}</h2><p>${esc(s.academicPeriod || "")} </p></div></div>
      </div>
      <div class="subjects-feature-summary">
        <div><span>Current grade</span><strong>—</strong></div>
        <div><span>Current unit</span><strong>${current ? `Unit ${esc(current.number)} — ${esc(current.name)}` : "—"}</strong></div>
      </div>
      <section class="subjects-feature-section">
        <div class="subjects-feature-section-heading"><div><h3>Units</h3><p>Organize the course into its units and track where you are.</p></div><button class="subjects-feature-button" id="unitAddButton">+ Add Unit</button></div>
        ${units.length ? `<div class="subjects-feature-unit-list">${units.map(unitCard).join("")}</div>` : `<div class="subjects-feature-empty"><strong>No units yet</strong>Add the first unit for ${esc(s.name)}.</div>`}
      </section>
    </div>`;
  ...
  function showAddUnit(subject) {
    const existing = data().units.filter(u => u.subjectId === subject.id);
    const nextNumber = existing.reduce((max, u) => Math.max(max, Number(u.number) || 0), 0) + 1;
    const b = document.createElement("div");
    b.className = "subjects-feature-modal-backdrop";
    b.innerHTML = `<div class="subjects-feature-modal" role="dialog" aria-modal="true" aria-label="Add Unit">
      <h2>Add Unit</h2>
      <form class="subjects-feature-form" id="unitForm">
        <div class="subjects-feature-field"><label for="unitName">Unit name</label><input id="unitName" required autofocus></div>
        <div class="subjects-feature-field"><label for="unitNumber">Unit number</label><input id="unitNumber" type="number" min="1" step="1" value="${nextNumber}" required></div>
        <div class="subjects-feature-field"><label>Status</label><div class="subjects-feature-status-options"><button type="button" data-status="Upcoming" class="selected">Upcoming</button><button type="button" data-status="Current">Current</button><button type="button" data-status="Completed">Completed</button></div></div>
        <div class="subjects-feature-actions"><button type="button" class="subjects-feature-secondary" id="unitCancel">Cancel</button><button class="subjects-feature-button" type="submit">Add Unit</button></div>
      </form>
    </div>`;
    document.body.appendChild(b);
    ...
    b.querySelector("#unitCancel").onclick = () => b.remove();
    b.onclick = e => { if (e.target === b) b.remove(); };
    b.querySelector("form").onsubmit = e => {
      e.preventDefault();
      const unit = window.PlannerData.create("units", { subjectId: subject.id, name: b.querySelector("#unitName").value.trim(), number: Number(b.querySelector("#unitNumber").value), status: selectedStatus });
      if (selectedStatus === "Current") {
        data().units.filter(u => u.subjectId === subject.id && u.id !== unit.id && u.status === "Current").forEach(u => window.PlannerData.update("units", u.id, { status: "Upcoming" }));
      }
      b.remove();
  ...
  window.addEventListener("planner:data-changed", () => { if (route()) render(); });
