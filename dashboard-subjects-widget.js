...
(function installDashboardSubjectWidget(){
    ...
    function render(){
        ...
        subjects.forEach(s=>{
            ...
            const row=document.createElement('div');row.className='dashboard-subject-row';row.tabIndex=0;row.setAttribute('role','button');row.setAttribute('aria-label',`Open ${s.name} subject page`);row.style.setProperty('--subject-color',s.colour||'var(--planner-accent, #304b8a)');
            ...
            row.onclick=e=>{if(e.target.closest('.dashboard-subject-task'))return;openSubject(s.name);};
            row.onkeydown=e=>{if((e.key==='Enter'||e.key===' ')&&!e.target.closest('.dashboard-subject-task')){e.preventDefault();openSubject(s.name);}};
