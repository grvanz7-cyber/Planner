...
(function(){
  ...
  function open(eventId=null,date=null){
    ...
    el('eventModalTitle').textContent=event?'Edit Event':'New Event';el('eventName').value=event?.name||'';el('eventDate').value=event?.date||date||'';el('eventTime').value=event?.time||'';el('eventNotes').value=event?.notes||'';el('eventColour').value=event?.colour||'#7c3aed';el('eventDelete').style.display=event?'':'none';el('eventModal').classList.add('open');el('eventName').focus();
  }
  ...
  function save(){
    ...
    if(!name){alert('Please enter an event name.');el('eventName').focus();return;}if(!date){alert('Please choose a date.');return;}
    ensure();const data={name,date,time:el('eventTime').value||'',notes:el('eventNotes').value.trim(),colour:el('eventColour').value||'#7c3aed'};
