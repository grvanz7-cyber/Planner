...
(function(){
  ...
  function ensurePage(){
    if(el('studyPage'))return;
    const nav=document.querySelector('.nav-section:nth-of-type(2)');
    if(nav&&!document.querySelector('[data-page="study"]')){
      const a=document.createElement('a');a.href='#study';a.className='nav-item';a.dataset.page='study';a.textContent='🧠 Study';a.onclick=()=>{showPage('study');return false;};
      nav.appendChild(a);
    }
    ...
    el('studyAddButton').onclick=openModal;el('studySubjectFilter').onchange=render;
  }
