...
(function(){
  ...
  function recordGrade(t){if(typeof window.openGradeModal!=='function'){alert('Grades is still loading. Please try again in a moment.');return;}window.openGradeModal();const select=el('gradeTask');if(select){select.value=String(t.id);select.dispatchEvent(new Event('change'));}}
  function renderAssessments(){
    ...
    list.innerHTML='';
    if(!filtered.length){list.innerHTML='<div class="empty-assessments">No assessments match your filters.</div>';if(typeof window.syncGradeButtons==='function')window.syncGradeButtons();return;}
    const subjects=plannerData?.settings?.subjects||[];
    filtered.forEach(t=>{const s=subjects.find(x=>x.name===t.subject),r=document.createElement('div');r.className='assessment-row '+state(t);r.dataset.taskId=t.id;r.innerHTML='<span class="assessment-icon"></span><span class="assessment-main"><strong></strong><small></small></span><span class="assessment-meta"><span class="assessment-type"></span><span class="assessment-due"></span><span class="assessment-weight"></span><button type="button" class="record-grade-button">Record Grade</button></span>';if(s?.colour)r.style.setProperty('--assessment-color',s.colour);r.querySelector('.assessment-icon').textContent=s?.emoji||typeEmoji(t.type);r.querySelector('strong').textContent=t.name||'Untitled assessment';r.querySelector('small').textContent=t.subject||'No subject';r.querySelector('.assessment-type').textContent=`${typeEmoji(t.type)} ${t.type||'Assessment'}`;r.querySelector('.assessment-due').textContent=t.dueDate?`Due ${t.dueDate}`:'No date';r.querySelector('.assessment-weight').textContent=t.weight!==''&&t.weight!=null?`${t.weight}%`:'';r.onclick=e=>{if(e.target.closest('.record-grade-button'))return;if(typeof openEditTaskModal==='function')openEditTaskModal(t.id);};r.querySelector('.record-grade-button').onclick=e=>{e.stopPropagation();recordGrade(t);};list.appendChild(r);});
