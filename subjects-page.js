...
function renderSubjectsPage() {
    ...
    activeSubjects.forEach(subject => {
        ...
        const subjectTasks = tasks.filter(t => String(t.subject||'').toLowerCase() === String(subject.name||'').toLowerCase());
        const activeTasks = subjectTasks.filter(t => !t.completed);
        const upcoming = activeTasks.filter(t => t.dueDate).sort((a,b)=>String(a.dueDate).localeCompare(String(b.dueDate))).slice(0,3);
